...
var showInspiration = function(u) {
	...
	// Adds the hyperlinked display name of the user
	var userElem = result.find('.username a');
	userElem.attr('href', u.user.link);
	userElem.text(u.user.display_name);

	// Adds the reputation of the user
	var rep = result.find('.reputation');
	rep.text(u.user.reputation);

	// Adds the post count of the user
	var counter = result.find('.post-count');
	counter.text(u.post_count);
